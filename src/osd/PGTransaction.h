...
public:
  ...
  public:
    ...
    bool is_delete() const {
      return boost::get<Init::Delete>(&init_type) != nullptr;
    }
    bool is_none() const {
	...
	[&](const Init::Delete &) -> bool { return false; });
    }
...
public:
  ...
  void create(
    ...
    auto iter = op_map.find(hoid);
    if (iter == op_map.end()) {
      ObjectOperation &op = op_map[hoid];
      op.init_type = ObjectOperation::Init::Create(false, boost::none);
    } else {
      assert(iter->second.is_delete());
      iter->second.init_type = ObjectOperation::Init::Create(true, boost::none);
    }
  }
  ...
  void clone(
    ...
    auto iter = op_map.find(target);
    if (iter == op_map.end()) {
      ObjectOperation &op = op_map[target];
      op.init_type = ObjectOperation::Init::Create(false, source);
    } else {
      assert(iter->second.is_delete());
      iter->second.init_type = ObjectOperation::Init::Create(true, source);
    }
  }
  ...
  void rename(
    ...
    assert(op_map.find(target) == op_map.end());
    auto iter = op_map.find(source);
    auto &op = op_map[target];
    if (iter != op_map.end()) {
      assert(iter->second.is_none());
      op = iter->second;
      op_map.erase(iter);
    }
  ...
  void remove(
    ...
    op = ObjectOperation();
    op.init_type = ObjectOperation::Init::Delete();
  }
